...
public class EvilBotQuienecence : IChessBot
{
    ...
    private int Quienscience(int depth, int alpha, int beta, Board studiedBoard)
    {
        if (depth == 0 || studiedBoard.GetLegalMoves(true).Length == 0)
        {
            return BoardEval(studiedBoard);
        }
        
        int standPat = BoardEval(studiedBoard);
        ...
        foreach (var capture in studiedBoard.GetLegalMoves(true))
        {
            studiedBoard.MakeMove(capture);
            int score = -Quienscience(depth-1, -alpha, -beta, studiedBoard);
            studiedBoard.UndoMove(capture);
